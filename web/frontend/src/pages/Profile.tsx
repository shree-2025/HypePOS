  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (mounted) setUser({ name: data?.name, email: data?.email, phone: data?.phone, role: data?.role })
      } catch {
        if (mounted) setUser({ name: 'User', email: 'user@example.com', phone: '', role: '' })
      } finally {
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-md bg-white shadow-soft p-4">
            ...
            <div className="mt-3 grid gap-3">
              ...
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="Your name" value={user?.name || ''} onChange={()=>{}} readOnly={loading} />
              <label className="text-sm text-gray-600">Email</label>
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="you@example.com" value={user?.email || ''} onChange={()=>{}} readOnly={loading} />
              <label className="text-sm text-gray-600">Phone</label>
              <input className="rounded-md border px-3 py-2 text-sm" placeholder="" value={user?.phone || ''} onChange={()=>{}} readOnly={loading} />
              <div className="pt-2">
